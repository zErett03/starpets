...
class GgselSellerOfficeClient:
    ...
    async def set_post_payment_url(self, offer_id: int, url: str) -> dict:
        # The offer's delivery mode is "auto" (ggsel coerces URL-delivered offers to auto). PATCHing
        # delivery="manual" tries to SWITCH modes → ggsel then demands a "check unique code url" and
        # returns 422 "Check unique code url имеет неверное значение". Send the offer's ACTUAL mode
        # ("auto") so we only update the URL — no mode switch, no unique-code validation.
        async with httpx.AsyncClient(headers=self._headers(), timeout=30) as client:
            resp = await self._request_retry(
                client, "PATCH", f"{SELLER_OFFICE_V2_URL}/offers/{offer_id}",
                json={"delivery": "auto", "post_payment_url": url}, retry_500=True,
            )
            if not resp.is_success:
