...
@app.get("/test-trade-status-paths")
async def test_trade_status_paths(id: str = "59510598"):
    """Diagnostic: probe candidate per-trade status paths (convention-consistent guesses).

    /trade/status returned a bare 404 (path doesn't exist). Every real endpoint
    contains 'ex-buyers', so probe the likely paths with ?tradeId= plus one path-param
    variant. A non-404 on any candidate means we found the per-trade endpoint.
    """
    candidates = [
        ("/ex-buyers/trades/status", {"tradeId": id}),
        ("/ex-buyers/trade/status", {"tradeId": id}),
        ("/trades/ex-buyers/status", {"tradeId": id}),
        (f"/ex-buyers/trades/{id}", None),  # id in path, no query param
    ]
    results = {}
    async with httpx.AsyncClient(timeout=15) as client:
        for path, extra in candidates:
            ...
            try:
                resp = await client.get(
                    f"{starpets.base_url}{path}", params=params, headers=headers
                )
                try:
                    body = resp.json()
                except Exception:
    ...
    return {"tradeId_tested": id, "results": results}
